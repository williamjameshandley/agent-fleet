                ...
                async with asyncio.timeout(30):
                    while True:
                        async with self.changed:
                            ...
                            if descriptors[0].get("state") == "retired":
                                raise RuntimeError(f"refreshed actor retired: {actor}")
                            if descriptors[0].get("state") == "unavailable":
                                providers = [current for group in self.sessions.values()
                                             for current in group
                                             if (current.ref == old_attachment
                                                 or (current.ref.server.host == host
                                                     and current.agent == session.agent
                                                     and current.transcript_id
                ...
                async with asyncio.timeout(30):
                    while True:
                        async with self.changed:
                            ...
                            if descriptors[0].get("state") == "retired":
                                raise RuntimeError(f"refreshed actor retired: {actor}")
                            matches = [current for group in self.sessions.values()
                                       for current in group if current.ref.key == key]
                            if len(matches) > 1:
                                raise RuntimeError(
                            ...
                            if matches and matches[0].attachment is not None:
                                [current] = matches
                                current_retained = {
                                    field: descriptors[0].get(field)
                                    for field in descriptor_fields}
                                if (current_retained != retained
                                        or current.agent != session.agent
